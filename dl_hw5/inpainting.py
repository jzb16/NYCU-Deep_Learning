    ...
    def inpainting(self, image, mask_b, i): #MakGIT inference
        ...
        with torch.no_grad():
            ...
            for step in range(self.total_iter):
                ...
                # Call the inpainting function from the model
                z_indices_predict, mask_bc = self.model.inpainting(z_indices, mask_bc, mask_num, ratio) # [1, 256]
                z_indices = z_indices_predict

                # Save mask and decoded images
...
if __name__ == '__main__':
    ...
    parser.add_argument('--partial', type=float, default=50, help='Number of epochs to train (default: 50)')    
...
#TODO3 step1-2: modify the path, MVTM parameters
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='./transformer_checkpoints/ckpt_epoch_square_9.pt', help='load ckpt')
    ...
    #MVTM parameter
    parser.add_argument('--sweet-spot', type=int, default=6, help='sweet spot: the best step in total iteration')
    ...
    for image, mask in zip(t.mi_ori, t.mask_ori):
        ...
        mask_b = t.get_mask_latent(mask) # mask_b [1, 256]
        maskgit.inpainting(image, mask_b, i)
        i += 1
        """if i % 100 == 0:
            print("inpainting: ", i)"""

# cosine 15, square 9, linear 18